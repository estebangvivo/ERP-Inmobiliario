...
export default async function RecibosPage({ searchParams }: PageProps) {
  ...
  const { items: receipts, total, page } = await listReceipts({
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Recibos"
        description="Cobros registrados en tesorería."
        actions={
          <Link href="/tesoreria/recibos/new">
            <Button size="sm">Nuevo recibo</Button>
          </Link>
        }
      />

      {total === 0 ? (
        <p className="text-sm text-[var(--muted-foreground)]">Todavía no hay recibos.</p>
      ) : (
        <>
          <ul className="divide-y divide-[var(--border)] border-y border-[var(--border)]">
            {receipts.map((item) => (
              <li key={item.id}>
                <Link
                  href={`/tesoreria/recibos/${item.id}`}
