...
/** Últimos cobros de cuotas que impactaron tesorería (sin recibo documental). */
export async function listRecentBillingPayments(limit = 8) {
  const session = await requireStaff();
  ...
  return rows.map((p) => ({
    ...
    method: p.method,
    partyName:
      p.tenantBill.contract.parties[0]?.user.name ??
      p.tenantBill.contract.code,
    label: `${p.tenantBill.periodMonth}/${p.tenantBill.periodYear} · ${p.tenantBill.contract.code}`,
    billId: p.tenantBillId,
  }));
}
